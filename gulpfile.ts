...
import * as packageImporter from 'node-sass-css-importer'
...
export function copyAssets() {
    return g.src(['front/images/**/*', 'front/fonts/**/*'], {base: 'front/'})
        .pipe(g.dest('public/assets/'))
}

export function buildSass() {
    return g.src('front/styles/**/*.sass')
        .pipe(plumber())
        .pipe(sass({
            importer: (packageImporter() as any)
        } as any))
        .pipe(autoprefixer())
...
export const build = g.parallel(runWebpack, buildSass, buildPug, gitbookBuild, copyAssets)

export default g.series(setDevFlag, g.parallel(build, gitbookWatch), runBrowserSync, watch)
